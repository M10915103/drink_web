...
export default function Footer(){
 ...
 return <footer id="contact" className="mana-footer shopify-section c-Footer" ref={root}>
  <div className="footer-socials">{[['fb','#social','Facebook'],['li','#social','LinkedIn'],['ig','#social','Instagram']].map(([icon,url,label])=><a href={url} key={icon} aria-label={label}><img src={assetUrl('i_'+icon+'.svg')} alt=""/></a>)}</div>
  <div className="game-landscape"><Lottie className="landscape-art" file="jeu_paysage.json"/><Lottie className={'game-runner '+(jumping?'jumping':'')} file={jumping?'jeu_jump.json':'jeu_walk.json'}/><img className="game-cloud" src={assetUrl('nuage.svg')} alt=""/></div>
  ...
  <div className="footer-bottom"><span>2026 © Tea House</span><div><a href="#contact">Terms of use</a><a href="#contact">Refund policy</a><button onClick={()=>setCredits(!credits)}>Credits</button></div></div>
